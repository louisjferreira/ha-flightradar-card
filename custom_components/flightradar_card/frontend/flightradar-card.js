...
class FlightRadarCard extends HTMLElement {
  ...
  static getConfigForm() {
    return {
      schema: [
        ...
        {
          name: "appearance.full_screen",
          selector: { boolean: {} },
        },
      ],
      computeLabel: (schema) => ({
        airport: "Tracked airport",
        "live.radius_nm": "Live aircraft radius (NM)",
        refresh_interval: "Refresh interval (seconds)",
        "map.zoom": "Map zoom",
        "appearance.full_screen": "Fill available screen height",
      }[schema.name]),
      computeHelper: (schema) => schema.name === "airport"
        ? "The airport is the map centre and the airport used for the live traffic area. The map itself displays all ADS-B aircraft returned within the selected radius."
        : undefined,
    };
  }
  ...
  setConfig(config) {
    ...
    this._config = {
      ...
      refresh_interval: 15,
      ...config,
      map: { zoom: 7, ...(config.map || {}) },
      appearance: { ...(config.appearance || {}) },
      live: { enabled: true, radius_nm: 250, ...(config.live || {}) },
    };
  ...
  _command(message) { return this._hass.callWS(message); }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .traffic-head{display:flex;justify-content:space-between;font-size:12px;font-weight:700;margin-bottom:8px}.traffic-sub{font-size:9px;color:#9aa6b2;font-weight:400}.traffic-row{display:grid;grid-template-columns:38px 72px 1fr 55px;gap:4px;align-items:center;background:#22282e;border-radius:8px;padding:8px;margin-top:4px;font-size:10px}.traffic-row .callsign{font-weight:700}.aircraft{position:absolute;z-index:6;cursor:pointer;color:white;font-size:24px;text-shadow:0 2px 8px #000}.aircraft.selected{color:#ffd33d;font-size:30px}.airport{position:absolute;z-index:5;transform:translate(-50%,-50%);width:12px;height:12px;background:white;border-radius:50%;box-shadow:0 0 0 3px rgba(255,255,255,.22)}.badge{position:absolute;z-index:12;bottom:10px;padding:8px 10px;border-radius:8px;background:rgba(17,22,27,.9);font-size:10px}.badge.left{left:10px}.badge.right{right:10px}.preview-label{color:#8ed0ff}.error{color:#ffb4ab;font-size:11px}
        @media(max-width:700px){.search{top:8px}.selected{top:72px;left:12px;width:calc(100% - 24px)}.traffic{top:calc(72px + 300px);left:12px;right:12px;width:auto}.traffic-row{grid-template-columns:34px 60px 1fr 48px}.badge{bottom:8px}}
      </style>
      <div class="card">
  ...
  _renderMap(airport, aircraft, selected) {
    ...
    for(let tx=tx0;tx<=tx1;tx++) for(let ty=ty0;ty<=ty1;ty++){
      ...
      img.style.left=`${tx*TILE+ox}px`; img.style.top=`${ty*TILE+oy}px`; frag.appendChild(img);
    }
    map.appendChild(frag);
    const airportEl=document.createElement("div"); airportEl.className="airport";
    airportEl.style.left=`${rect.width/2}px`; airportEl.style.top=`${rect.height/2}px`; map.appendChild(airportEl);
    for(const f of aircraft){
      if(!Number.isFinite(f.lat)||!Number.isFinite(f.lon)) continue;
      const p=project(f.lat,f.lon); const el=document.createElement("div");
      const selectedId=selected?.hex||selected?.icao24; const id=f.hex||f.icao24;
      el.className=`aircraft ${selectedId===id?"selected":""}`; el.textContent="✈";
      ...
      el.addEventListener("click",()=>{this._selected=f;this._render()}); map.appendChild(el);
    }
  }
  ...
  _renderTraffic(airport, aircraft) {
    ...
    panel.innerHTML=`<div class="traffic-head"><span>${airport.code} · Live Air Traffic</span><span class="traffic-sub">ALL AIRCRAFT IN AREA</span></div>${rows||`<div class="traffic-row"><span>—</span><span class="callsign">NO DATA</span><span>${this._loading?"Waiting for ADS-B":(this._error||"No aircraft")}</span><span>—</span></div>`}`;
  }
}
