...
class FlightRadarCard extends HTMLElement {
  ...
  async _fetchAircraft() {
    ...
    try {
      ...
      if (this._selected) {
        ...
        this._selected = this._flights.find((f) => (f.hex || f.icao24) === selectedId) || this._selected;
      }
    } catch (err) {
  ...
  _command(message) {
    return new Promise((resolve, reject) => {
      this._hass.callWS(message).then(resolve).catch(reject);
    });
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .map { position:absolute; inset:0; overflow:hidden; background:#b8c3c8; }
        .map img { position:absolute; width:256px; height:256px; image-rendering:auto; }
        .overlay { position:absolute; inset:0; background:linear-gradient(180deg,rgba(0,0,0,.24),rgba(0,0,0,.08) 45%,rgba(0,0,0,.34)); pointer-events:none; }
        ...
        .selected { top:12px; left:12px; width:min(300px,calc(100% - 24px)); overflow:hidden; }
        .traffic { top:12px; right:12px; width:min(330px,calc(100% - 24px)); padding:12px; }
        ...
        .search input { flex:1; min-width:0; background:#252b31; color:#fff; border:0; border-radius:8px; padding:10px 12px; outline:none; }
        .search button { border:0; border-radius:8px; padding:0 14px; font-weight:700; cursor:pointer; }
        .photo { width:100%; height:105px; object-fit:cover; display:block; }
        .content { padding:12px; }
        .eyebrow { font-size:10px; letter-spacing:1.6px; color:#aeb9c4; }
        h2 { margin:4px 0; font-size:23px; }
        .airline { color:#c6ced6; font-size:12px; }
        .route { display:flex; align-items:center; gap:8px; margin:12px 0; font-size:12px; }
        .route span:nth-child(2) { flex:1; height:1px; background:#59636c; }
        .stats { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; }
        .stat { background:#252b31; padding:8px; border-radius:9px; }
        .stat label { display:block; font-size:9px; color:#89949f; text-transform:uppercase; }
        .stat b { font-size:12px; }
        .traffic-head { display:flex; justify-content:space-between; font-size:12px; font-weight:700; margin-bottom:8px; }
        .traffic-row { display:grid; grid-template-columns:38px 62px 1fr 48px; gap:4px; align-items:center; background:#22282e; border-radius:8px; padding:8px; margin-top:4px; font-size:10px; }
        .traffic-row .callsign { font-weight:700; }
        .aircraft { position:absolute; z-index:6; transform:translate(-50%,-50%); cursor:pointer; color:white; font-size:24px; text-shadow:0 2px 8px #000; }
        .aircraft.selected { color:#ffd33d; font-size:30px; }
        .airport { position:absolute; z-index:5; transform:translate(-50%,-50%); width:12px; height:12px; background:white; border-radius:50%; box-shadow:0 0 0 3px rgba(255,255,255,.22); }
        .badge { position:absolute; z-index:12; bottom:10px; padding:8px 10px; border-radius:8px; background:rgba(17,22,27,.9); font-size:10px; }
        .badge.left { left:10px; }.badge.right { right:10px; }
        .error { color:#ffb4ab; font-size:11px; }
      ...
      <div class="card">
        ...
        <div class="badge left">${airport.name}</div>
        <div class="badge right">${aircraft.length ? `${aircraft.length} aircraft · LIVE ADS-B` : "Live ADS-B unavailable"}</div>
      </div>`;
  ...
  _renderMap(airport, aircraft, selected) {
    ...
    const airportPos = project(airport.lat, airport.lon);
    const airportEl = document.createElement("div");
    airportEl.className = "airport";
    airportEl.style.left = `${airportPos.x + ox}px`;
    airportEl.style.top = `${airportPos.y + oy}px`;
    ...
    for (const f of aircraft) {
      ...
      el.className = `aircraft ${(selected?.hex || selected?.icao24) === (f.hex || f.icao24) ? "selected" : ""}`;
      el.textContent = "✈";
      el.title = f.callsign || f.registration || f.hex || "Aircraft";
  ...
  _renderSelected(f) {
    ...
    if (!panel) return;
    const callsign = f?.callsign || f?.registration || f?.hex || "—";
    const airline = f?.airline || AIRLINES[String(callsign).replace(/\d/g, "")] || "Live ADS-B aircraft";
    panel.innerHTML = `<div class="content"><div class="eyebrow">SELECTED AIRCRAFT</div><h2>${callsign}</h2><div class="airline">${airline}</div><div class="route"><span>${f?.origin || "—"}</span><span></span><span>${f?.destination || "—"}</span></div><div class="stats"><div class="stat"><label>Aircraft</label><b>${f?.type || "—"}</b></div><div class="stat"><label>Altitude</label><b>${f?.altitude != null ? `${Math.round(f.altitude).toLocaleString()} ft` : "—"}</b></div><div class="stat"><label>Speed</label><b>${f?.speed != null ? `${Math.round(f.speed)} km/h` : "—"}</b></div></div></div>`;
  }

  _renderTraffic(aircraft) {
    const panel = this.shadowRoot.getElementById("trafficPanel");
    if (!panel) return;
    const rows = aircraft.slice(0, 7).map((f) => `<div class="traffic-row"><span>ADS-B</span><span class="callsign">${f.callsign || f.registration || "—"}</span><span>${f.type || "Unknown"}</span><span>${f.altitude != null ? `${Math.round(f.altitude / 100) * 100} ft` : "—"}</span></div>`).join("");
