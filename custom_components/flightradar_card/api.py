...
# Public ADS-B feeds. Keep multiple sources because coverage varies heavily by
# region (especially Southern Africa), and any one community feed can be down.
PROVIDERS = (
...
async def get_aircraft(hass, latitude: float, longitude: float, radius: int) -> dict[str, Any]:
    ...
    for name, base, provider_type in PROVIDERS:
        try:
            ...
            # A provider must never prevent the next fallback from being tried.
            errors.append(f"{name}: {type(err).__name__}: {err}")
...
async def search_aircraft(hass, query: str) -> dict[str, Any]:
    ...
    errors: list[str] = []
    for name, base, _provider_type in PROVIDERS:
